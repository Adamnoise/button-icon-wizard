import { FigmaNode, Color, Paint, Rectangle, TypeStyle, LayoutConstraint, NodeType } from '../types/figma';
...
export default ${componentName};`;
  ...
  private generateJSXFromNode(node: FigmaNode, indent: number = 0): string {
    ...
    if (node.type === 'IMAGE') {
      // Feltételezzük, hogy az imageRef egy URL-re mutat, vagy placeholder
      const imgSrc = node.imageRef ? `/api/images/${node.imageRef}` : '/placeholder.svg';
      return `${indentStr}<img className="${className}" src="${imgSrc}" alt="${node.name || 'Image'}" />`;
    }

    if (node.type === 'VECTOR') {
      // A vectorData-t a Figma API-ból kellene kinyerni, itt csak placeholder
      const svgContent = node.vectorData || `<!-- SVG content for ${node.name} would go here -->`;
