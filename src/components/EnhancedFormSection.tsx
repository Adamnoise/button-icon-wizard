...
const useFormValidation = (apiService: FigmaApiService | null) => {
  ...
  const validateToken = useCallback((token: string): ValidationState => {
...
const useConversion = (
  ...
  const conversionSteps = useMemo(() => [
    { pct: 15, label: 'Figma API kapcsolat létrehozása...', step: 0 },
    { pct: 30, label: 'Design fájl letöltése és elemzése...', step: 1 },
    { pct: 50, label: 'Szemantikus struktúra feltérképezése...', step: 2 },
    { pct: 70, label: 'CSS és layout generálása...', step: 3 },
    { pct: 85, label: 'JavaScript interakciók létrehozása...', step: 4 },
    { pct: 95, label: 'Kód optimalizálás és validálás...', step: 5 },
    { pct: 100, label: 'Exportálás befejezése...', step: 6 }
  ], []);

  const handleConvert = useCallback(async (figmaUrl: string, figmaToken: string) => {
    if (!apiService?.validateUrl(figmaUrl) || !apiService?.validateToken(figmaToken)) {
      showToast('Érvényes Figma URL és API token szükséges');
      return;
    }
    ...
    try {
      ...
      console.log('Konvertálás indítása:', { fileId, tokenLength: figmaToken.length });
      
      try {
        const figmaFile = await apiService.getFile(fileId);
        console.log('Fájl sikeresen betöltve:', figmaFile.name);
        
        const codeGenerator = new CodeGenerator(figmaFile, config);
...
<body>
  ...
  }, [apiService, config, conversionSteps, showToast, onConversionComplete]);
...
const EnhancedFormSection: React.FC<EnhancedFormSectionProps> = ({ 
  ...
  const { conversionState, handleConvert } = useConversion(apiService, config, showToast, onConversionComplete);

  // Initialize API service
  React.useEffect(() => {
    if (formState.figmaToken && validateToken(formState.figmaToken).isValid) {
      try {
        const newApiService = new FigmaApiService();
        newApiService.setToken(formState.figmaToken);
        setApiService(newApiService);
      } catch (error) {
        console.error('API szolgáltatás inicializálása sikertelen:', error);
        setApiService(null);
      }
    } else {
      setApiService(null);
    }
  }, [formState.figmaToken, validateToken]);

  // Validation states
  const urlValidation = useMemo(() => validateUrl(formState.figmaUrl), [formState.figmaUrl, validateUrl]);
  const tokenValidation = useMemo(() => validateToken(formState.figmaToken), [formState.figmaToken, validateToken]);
  ...
  const handlePaste = useCallback(async (setter: (value: string) => void) => {
    try {
      ...
      console.error('Beillesztés sikertelen:', error);
      showToast('Beillesztés sikertelen - engedély szükséges');
    }
  }, [showToast]);
  ...
  const handleConvertClick = useCallback(() => {
    handleConvert(formState.figmaUrl, formState.figmaToken);
  }, [handleConvert, formState.figmaUrl, formState.figmaToken]);
        ...
        <form className="mt-6 grid grid-cols-1 gap-5" onSubmit={(e) => e.preventDefault()}>
          ...
          <div>
            ...
            <div className="mt-2 space-y-1">
              ...
              {formState.figmaToken && !tokenValidation.isValid && (
          ...
          {!canConvert && (formState.figmaUrl || formState.figmaToken) && (
